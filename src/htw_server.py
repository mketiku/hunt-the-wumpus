...
'''Application serves a game of hunt the wumpus using a socket.'''
__author__ = "Michael Ketiku"
__project__ = "HuntTheWumpusServer"
...
class HTWServer:
    """HTWServer deals with networking and communication with the HTWClient."""
...
class HTWServerGame(HTWServer):
    """HTWServerGame deals with the game logic on the server side."""

    def __init__(self):
        """Initializes the server game object."""
        HTWServer.__init__(self)
    ...
    def __main_loop(self):
        ...
        while True:
            ...
            # Initialize a new Player object to store all the client's
            # infomation
        ...
        try:
            ...
            while player.is_waiting:
                ...
                if(match_result is None):
                    ...
                    # Initialize a new Game object to store the game's
                    # infomation
                    new_game = Game()
                    # Assign both players
                    new_game.player1 = player
                    new_game.player2 = match_result
                    # Create an empty string for empty room content
                    new_game.room_content = list("         ")
    ...
    def matching_player(self, player):
        ...
        try:
            ...
            for p in self.waiting_players:
                # If another player is found waiting and its not the player
                # itself
...
class Player:
    ...
    def send_match_info(self):
        """Sends the matched information to the client, which includes
...
class Game:
    ...
    def move(self, moving_player, waiting_player):
        ...
        # Send both players the current room content
        moving_player.send("B", ("".join(self.room_content)))
        waiting_player.send("B", ("".join(self.room_content)))
        ...
        if(self.room_content[move - 1] == " "):
            # Write the it into the room
            self.room_content[move - 1] = moving_player.role
        else:
        ...
        if(result >= 0):
            ...
            # Send back the latest room content
            moving_player.send("B", ("".join(self.room_content)))
            waiting_player.send("B", ("".join(self.room_content)))
