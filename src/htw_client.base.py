    ...
    def connect(self, address, port_number):
        ...
        while True:
            try:
                ...
                # Caught an error
                print("There was an error when trying to connect to " +
...
class HTWClientGame(HTWClient):
    """HTWClientGame deals with the game logic on the client side."""

    def __init__(self):
        """Initializes the client game object."""
        HTWClient.__init__(self)
    ...
    def __connected__(self):
        ...
        # Welcome the user
        print("Welcome to Hunt The Wumpus online, player " + str(self.player_id)
    ...
    def __game_started__(self):
        ...
        # This is a virtual function that does nothing right now but may be used
        # in a gui to let users know the status of the game.
        return

    def __main_loop(self):
        """The main game loop."""
        while True:
            # Get the room content from the server
            room_content = self.s_recv(10, "B")
            # Get the command from the server
            command = self.s_recv(2, "C")
            # Update the room
            self.__update_room__(command, room_content)

            if(command == "Y"):
                # If it's this player's turn to move
                self.__player_move__(room_content)
            elif(command == "N"):
    ...
    def __update_room__(self, command, room_string):
        """(Private) Updates the room."""
        if(command == "Y"):
            # If it's this player's turn to move, print out the current
            # room with " " converted to the corresponding position number
            print("Current room:\n" + HTWClientGame.format_room(
                HTWClientGame.show_room_pos(room_string)))
        else:
            # Print out the current room
            print("Current room:\n" + HTWClientGame.format_room(
                room_string))

    def __player_move__(self, room_string):
        ...
        while True:
            ...
            if(position >= 1 and position <= 9):
                ...
                print("Please enter a value between 1 and 9 that" +
                      "corresponds to the position on the grid room.")
    ...
    def show_room_pos(s):
        """(Static) Converts the empty positions " " (a space) in the room
